...
export const useResponsiveGrid = (options: Partial<ResponsiveGridOptions> = {}) => {
  ...
  const calculateOptimalColumns = useCallback((containerWidth: number) => {
    ...
    const getBreakpointColumns = (width: number): { min: number; max: number; ideal: number } => {
      if (width < 480) {
    ...
    for (let columns = breakpoint.min; columns <= Math.min(breakpoint.max, config.maxColumns); columns++) {
      ...
      if (columnWidth >= config.minCardWidth && columnWidth <= config.maxCardWidth) {
    ...
    if (bestScore === Infinity) {
      const maxPossibleColumns = Math.floor(availableWidth / (config.minCardWidth + config.gap));
      bestColumns = Math.max(breakpoint.min, Math.min(maxPossibleColumns, breakpoint.max, config.maxColumns));
    }
